...
function draw(){ ctx.clearRect(0,0,canvas.width,canvas.height); ctx.save(); ctx.font='bold 28px Arial'; ctx.fillStyle='#4a148c'; ctx.textAlign='center'; ctx.fillText('Cazador de Números', canvas.width/2,40); ctx.font='20px Arial'; ctx.fillStyle='#333'; ctx.fillText('Objetivo: '+(targetOp?targetOp.texto+' = ?':''), canvas.width/2,70); ctx.fillText('Tiempo: '+tiempo.toFixed(1)+'  Puntos: '+score+'  Récord: '+highScore, canvas.width/2,100); if(!started) { if(tiempo<=0) { ctx.fillStyle='#2e7d32'; ctx.fillText('Fin. Pulsa cualquier tecla para reiniciar.', canvas.width/2,140);} else ctx.fillText('Pulsa cualquier tecla para comenzar', canvas.width/2,140);} ctx.restore(); for(let f of fallers){ ctx.save(); ctx.beginPath(); ctx.arc(f.x,f.y,24,0,Math.PI*2); ctx.fillStyle = (f.val===targetOp.res)?'#81c784':'#ffcc80'; ctx.fill(); ctx.fillStyle='#000'; ctx.font='16px Arial'; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText(f.val, f.x, f.y); ctx.restore(); } }
...
function key(e){ if(!started){ if(tiempo<=0 && score>highScore){ highScore=score; localStorage.setItem('mathCatcherHigh', String(highScore)); } started=true; tiempo=60; score=0; fallers=[]; nuevaOperacion(); return; }}
function click(e){ if(!started) return; const rect=canvas.getBoundingClientRect(); const mx=e.clientX-rect.left, my=e.clientY-rect.top; for(let i=fallers.length-1;i>=0;i--){ const f=fallers[i]; const dx=mx-f.x, dy=my-f.y; if(dx*dx+dy*dy<24*24){ if(f.val===targetOp.res){ score+=50; if(score>highScore){ highScore=score; localStorage.setItem('mathCatcherHigh', String(highScore)); } nuevaOperacion(); fallers=[]; } else score=Math.max(0,score-20); fallers.splice(i,1); break;} }
}
