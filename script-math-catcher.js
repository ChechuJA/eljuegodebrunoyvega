...
function key(e){ if(!started){ started=true; tiempo=60; score=0; fallers=[]; nuevaOperacion(); return; }}
function click(e){ if(!started) return; const rect=canvas.getBoundingClientRect(); const mx=e.clientX-rect.left, my=e.clientY-rect.top; for(let i=fallers.length-1;i>=0;i--){ const f=fallers[i]; const dx=mx-f.x, dy=my-f.y; if(dx*dx+dy*dy<24*24){ if(f.val===targetOp.res){ score+=50; nuevaOperacion(); fallers=[]; } else score=Math.max(0,score-20); fallers.splice(i,1); break;} }
}
