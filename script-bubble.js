...
function drawHUD(){ if(window.GameUI) GameUI.gradientBar(ctx,canvas.width,60,'#0d47a1','#1976d2'); else { ctx.fillStyle='#0d47a1'; ctx.fillRect(0,0,canvas.width,60);} ctx.fillStyle='#fff'; ctx.font='bold 22px Arial'; ctx.textAlign='left'; ctx.fillText('Puntos: '+score,14,38); ctx.textAlign='center'; ctx.fillText('Dispara Colores', canvas.width/2,38); ctx.textAlign='right'; ctx.fillText('Récord: '+high+' ('+highName+')', canvas.width-14,38); }
function drawIntro(){ ctx.save(); const w=canvas.width-140; const h=240; const x=70; const y=90; if(window.GameUI) GameUI.glassPanel(ctx,x,y,w,h); else { ctx.globalAlpha=0.9; ctx.fillStyle='#fff'; ctx.fillRect(x,y,w,h); ctx.globalAlpha=1;} ctx.fillStyle='#0d47a1'; ctx.font='bold 28px Arial'; ctx.textAlign='center'; ctx.fillText('Dispara Colores', canvas.width/2,y+50); ctx.font='15px Arial'; ctx.fillStyle='#333'; ctx.fillText('Apunta con el ratón y haz clic para disparar.', canvas.width/2,y+88); ctx.fillText('Junta 4 o más del mismo color para sumar.', canvas.width/2,y+112); ctx.fillText('Evita que las filas bajen demasiado.', canvas.width/2,y+136); ctx.fillText('Sesiones cortas (máx. 10 min).', canvas.width/2,y+160); ctx.fillText('Pulsa cualquier tecla para empezar.', canvas.width/2,y+190); ctx.restore(); }
function draw(){ ctx.clearRect(0,0,canvas.width,canvas.height); if(window.GameUI) GameUI.softBg(ctx,canvas.width,canvas.height,['#e3f2fd','#bbdefb']); else { ctx.fillStyle='#e3f2fd'; ctx.fillRect(0,0,canvas.width,canvas.height);} drawGrid(); drawShooter(); drawHUD(); if(showIntro) drawIntro(); }
