...
placeItems();
function draw(){ ctx.clearRect(0,0,canvas.width,canvas.height); for(let r=0;r<rows;r++){ for(let c=0;c<cols;c++){ if(grid[r][c]===1){ ctx.fillStyle='#263238'; ctx.fillRect(c*cell,r*cell,cell,cell);} }} ctx.save(); ctx.font='20px serif'; ctx.textAlign='center'; ctx.textBaseline='middle'; for(let it of items){ ctx.fillText(it.icon,it.c*cell+cell/2,it.r*cell+cell/2); } ctx.restore(); ctx.fillStyle='#ff5722'; ctx.fillRect(player.c*cell+6,player.r*cell+6,cell-12,cell-12); ctx.fillStyle='#fff'; ctx.font='16px Arial'; ctx.fillText('Objetivo: '+objetivoSecuencia[idx], canvas.width-110,20); ctx.fillText('Pasos: '+player.steps+'  Mejor: '+(bestSteps>0?bestSteps:'-'), 110,20); }
