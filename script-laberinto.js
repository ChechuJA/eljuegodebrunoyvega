...
const player={c:1,r:1}; grid[player.r][player.c]=0; const objetivoSecuencia=['🔴','🟢','🔵','🟡']; let idx=0; let items=[];
...
placeItems();
function draw(){ ctx.clearRect(0,0,canvas.width,canvas.height); for(let r=0;r<rows;r++){ for(let c=0;c<cols;c++){ if(grid[r][c]===1){ ctx.fillStyle='#263238'; ctx.fillRect(c*cell,r*cell,cell,cell);} }} ctx.save(); ctx.font='20px serif'; ctx.textAlign='center'; ctx.textBaseline='middle'; for(let it of items){ ctx.fillText(it.icon,it.c*cell+cell/2,it.r*cell+cell/2); } ctx.restore(); ctx.fillStyle='#ff5722'; ctx.fillRect(player.c*cell+6,player.r*cell+6,cell-12,cell-12); ctx.fillStyle='#fff'; ctx.font='16px Arial'; ctx.fillText('Objetivo: '+objetivoSecuencia[idx], canvas.width-110,20); }
function canMove(c,r){ return c>=0&&c<cols&&r>=0&&r<rows&&grid[r][c]===0; }
function key(e){ let nc=player.c, nr=player.r; if(e.key==='ArrowLeft') nc--; else if(e.key==='ArrowRight') nc++; else if(e.key==='ArrowUp') nr--; else if(e.key==='ArrowDown') nr++; if(canMove(nc,nr)){ player.c=nc; player.r=nr; const item = items.find(i=>i.c===player.c&&i.r===player.r); if(item){ if(item.icon===objetivoSecuencia[idx]){ idx++; items=items.filter(it=>it!==item); if(idx===objetivoSecuencia.length){ setTimeout(()=>{ alert('¡Laberinto completado!'); idx=0; placeItems(); },50); } } }
 } }
