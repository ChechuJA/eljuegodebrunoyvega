...
let idx=0; let current=figuras[idx]; let progreso=0; let completadas=0; let mostrarInicio=true; let infoTimer=0; let mejorCompletadas=Number(localStorage.getItem('constelMejor')||0);
...
function drawConstelacion(){ ctx.save(); ctx.strokeStyle='#90caf9'; ctx.lineWidth=2; ctx.beginPath(); for(let i=0;i<progreso;i++){ const [x,y]=current.puntos[i]; if(i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y);} ctx.stroke(); ctx.fillStyle='#fff'; for(let i=0;i<current.puntos.length;i++){ const [x,y]=current.puntos[i]; ctx.beginPath(); ctx.arc(x,y, i===progreso?8:5,0,Math.PI*2); ctx.fillStyle = i<progreso? '#ffeb3b':'#fff'; ctx.fill(); }
 ...
 ctx.restore(); }
function drawHUD(){ ctx.save(); ctx.font='bold 26px Arial'; ctx.fillStyle='#fff'; ctx.textAlign='center'; ctx.fillText('Constelaciones', canvas.width/2,40); ctx.font='16px Arial'; ctx.fillText('Figura: '+current.nombre+'  ('+(idx+1)+'/'+figuras.length+')', canvas.width/2,64); ctx.fillText('Completadas: '+completadas+'  Récord: '+mejorCompletadas, canvas.width/2,84); if(mostrarInicio) ctx.fillText('Clic en cada estrella en orden para dibujar la constelación', canvas.width/2,108); if(infoTimer>0){ ctx.fillStyle='#ffeb3b'; ctx.fillText(current.dato, canvas.width/2, 128);} ctx.restore(); }
function loop(){ drawFondo(); drawConstelacion(); drawHUD(); if(infoTimer>0) infoTimer-=0.016; af=requestAnimationFrame(loop); }
function click(e){ const rect=canvas.getBoundingClientRect(); const mx=e.clientX-rect.left, my=e.clientY-rect.top; mostrarInicio=false; const [x,y]=current.puntos[progreso]; const dx=mx-x, dy=my-y; if(dx*dx+dy*dy<14*14){ progreso++; if(progreso===current.puntos.length){ completadas++; if(completadas>mejorCompletadas){ mejorCompletadas=completadas; localStorage.setItem('constelMejor', String(mejorCompletadas)); } infoTimer=3; setTimeout(()=>{ idx=(idx+1)%figuras.length; current=figuras[idx]; progreso=0; },1500); } } }
