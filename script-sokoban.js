function registerGame(){
	...
	canvas.width=mapaBase[0].length*TILE; canvas.height=mapaBase.length*TILE+80;
	...
	function mover(dx,dy){ const nx=jugador.x+dx, ny=jugador.y+dy; if(mapa[ny][nx]==='#') return; const c=cajaEn(nx,ny); if(c){ const nx2=nx+dx, ny2=ny+dy; if(mapa[ny2][nx2]==='#'||cajaEn(nx2,ny2)) return; c.x=nx2; c.y=ny2; } jugador.x=nx; jugador.y=ny; movimientos++; if(completado()){ if(mejor===0||movimientos<mejor){ mejor=movimientos; mejorName=playerName||'-'; localStorage.setItem('sokobanBest', String(mejor)); localStorage.setItem('sokobanBestName', mejorName); } setTimeout(()=>{ alert('Nivel completado en '+movimientos+' movimientos'); reiniciar(); },50); } }
	function completado(){ return cajas.every(c=>isObjetivo(c.x,c.y)); }
	function reiniciar(){ parse(); movimientos=0; }
	function draw(){ ctx.clearRect(0,0,canvas.width,canvas.height); if(window.GameUI) GameUI.softBg(ctx,canvas.width,canvas.height,['#eceff1','#cfd8dc']); if(window.GameUI){ GameUI.gradientBar(ctx,canvas.width,72,'#0d47a1','#1976d2'); } else { let g=ctx.createLinearGradient(0,0,0,70); g.addColorStop(0,'#0d47a1'); g.addColorStop(1,'#1976d2'); ctx.fillStyle=g; ctx.fillRect(0,0,canvas.width,70);} ctx.fillStyle='#fff'; ctx.font='22px Arial'; ctx.textBaseline='middle'; ctx.textAlign='left'; ctx.fillText('Empuja Cajas', 14,26); ctx.font='14px Arial'; ctx.fillText('Movs: '+movimientos,14,52); ctx.textAlign='right'; ctx.fillText('Récord: '+(mejor>0?mejor:'-')+' ('+mejorName+')', canvas.width-12,26); ctx.fillText('(R) Reiniciar', canvas.width-12,52); ctx.textAlign='left'; for(let y=0;y<mapa.length;y++) for(let x=0;x<mapa[0].length;x++){ const ch=mapa[y][x]; const baseY=y*TILE+72; if(ch==='#'){ ctx.fillStyle='#455a64'; ctx.fillRect(x*TILE,baseY,TILE,TILE); } else { ctx.fillStyle='#fafafa'; ctx.fillRect(x*TILE,baseY,TILE,TILE); if(isObjetivo(x,y)){ ctx.fillStyle='#ffca28'; ctx.beginPath(); ctx.arc(x*TILE+TILE/2,baseY+TILE/2,8,0,Math.PI*2); ctx.fill(); } } } for(let c of cajas){ ctx.fillStyle='#8d6e63'; ctx.fillRect(c.x*TILE+4,c.y*TILE+76,TILE-8,TILE-8); } ctx.fillStyle='#4caf50'; ctx.beginPath(); ctx.arc(jugador.x*TILE+TILE/2, jugador.y*TILE+72+TILE/2, TILE/3,0,Math.PI*2); ctx.fill(); }
